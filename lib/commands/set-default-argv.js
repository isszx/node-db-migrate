...
module.exports = function (internals, isModule) {
  ...
  if (is.Electron && is.Linux  && process.env.NODE_ENV !== 'development') {
    defaultConfig['config'] = internals.configFile || path.join(process.execPath, '..', 'database.json');
    defaultConfig['migrations-dir'] = path.join(process.execPath, '..', 'migrations');
    defaultConfig['vcseeder-dir'] = path.join(process.execPath, '..', 'VCSeeder');
    defaultConfig['staticseeder-dir'] = path.join(process.execPath, '..', 'Seeder');
  }
